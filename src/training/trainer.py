...
def init_wandb(config, project_name, run_name=None):
    ...
    """
    # Import lazily: chỉ ai bật W&B mới cần cài wandb (Colab/CI không cần).
    import wandb
...
def train_model(
    ...
    training_args = create_training_arguments(
        ...
        # Phải tôn trọng use_wandb ở đây nữa: nếu chỉ tắt init_wandb() mà vẫn
        # chuyển config xuống, WandbCallback của Trainer sẽ tự init và đòi API key.
        wandb_config=training_config.get("wandb") if use_wandb else None,
    )
